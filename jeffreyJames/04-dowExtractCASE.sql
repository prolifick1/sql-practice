...
CASE
  ...
  WHEN t.dow = 4 THEN 'Thursday'
  WHEN t.dow = 5 THEN 'Friday'
  WHEN t.dow = 6 THEN 'Saturday'
END
FROM (SELECT extract(dow FROM p.payment_date) dow, SUM(p.amount)::money FROM payment p GROUP BY dow ORDER BY dow) t;
